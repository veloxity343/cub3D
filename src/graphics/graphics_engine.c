...
/**
 * @brief Calculates where the ray hit the wall along the texture's width (X-axis).
 * @details Also flips the texture if the ray hits the wall from the opposite side.
 */
static void handle_tex_dir(t_ray *ray, t_tex *tex)
{
...
/**
 * @brief Sets up how much to advance down in the texture (Y-axis) per screen pixel
 ...
 * Finds texture position based on the wall slice's vertical position from midpoint.
 */
static void init_tex_step_pos(t_ray *ray, t_tex *tex, t_game *game)
{
